...
export default function ClubMessagesPage() {
    ...
    const handleDeleteConversation = async () => {
        ...
        try {
            const res = await fetch(`/api/chat/conversations?conversationId=${activeConversation._id}`, { method: "DELETE" });
            const data = await res.json();
            if (data.success) {
        ...
        } catch { toast.error("Error deleting conversation"); }
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (!data.success) { toast.error("Failed to send message"); fetchMessages(activeConversation._id); }
    ...
    const renderMessageContent = (content, isMe = false) => {
        ...
        return content.split(urlRegex).map((part, i) =>
            part.match(urlRegex)
                ? <a key={i} href={part} target="_blank" rel="noopener noreferrer" className={`underline break-all ${isMe ? 'text-indigo-200 hover:text-white' : 'text-indigo-600 hover:text-indigo-800'}`} onClick={(e) => e.stopPropagation()}>{part}</a>
                : part
        );
    };
    ...
    const getAvatarColor = (name) => {
        const palettes = ['bg-indigo-500', 'bg-purple-500', 'bg-pink-500', 'bg-blue-500', 'bg-teal-500', 'bg-violet-500', 'bg-cyan-500'];
    ...
    const handleContextMenu = (e, msg) => {
        if (msg.sender !== (user.id || user._id) || msg.isDeleted) return;
        e.preventDefault();
    ...
    const handleStartEdit = (msg) => { setEditingMsg(msg); setEditContent(msg.content); closeContextMenu(); };

    const handleSaveEdit = async () => {
        if (!editContent.trim() || !editingMsg) return;
        try {
            const res = await fetch("/api/chat/messages", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messageId: editingMsg._id, content: editContent }) });
            const data = await res.json();
            if (data.success) { setMessages(prev => prev.map(m => m._id === editingMsg._id ? data.message : m)); setEditingMsg(null); setEditContent(""); }
            else toast.error(data.message || "Failed to edit message");
    ...
    const handleDeleteMsg = async (msgId) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) setMessages(prev => prev.map(m => m._id === msgId ? data.message : m));
    ...
    const filteredConversations = filterQuery
        ? conversations.filter(c => (getPartner(c)?.name || "").toLowerCase().includes(filterQuery.toLowerCase()))
        : conversations;
                    ...
                    {filteredConversations.length > 0 ? (
                        filteredConversations.map(conv => {
                            ...
                            return (
                                <div key={conv._id} onClick={() => handleSelectConversation(conv)} className={`px-4 py-3 cursor-pointer flex items-center gap-3 transition-all ${isActive ? 'bg-indigo-50 border-r-[3px] border-indigo-500' : 'hover:bg-slate-50 border-b border-slate-100'}`}>
                                    <div className={`w-11 h-11 rounded-full ${getAvatarColor(partner?.name)} shrink-0 flex items-center justify-center text-white font-bold text-base shadow-sm`}>
                                        {partner?.name?.charAt(0) || "?"}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-baseline mb-0.5">
                                            <h3 className={`truncate text-sm ${hasUnread ? 'text-slate-900 font-bold' : 'text-slate-700 font-semibold'}`}>{partner?.name || "Student"}</h3>
                        ...
                        <div className="p-10 text-center flex flex-col items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">Student messages will appear here.</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {bulkMode ? (
                    ...
                    <div className="text-center z-10 p-8 flex flex-col items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-400 max-w-xs mx-auto leading-relaxed">Select a conversation to reply to student inquiries.</p>
                        </div>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="h-16 px-4 bg-white border-b border-slate-200 flex items-center justify-between z-10 shrink-0 shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <>
                                            <div className={`w-10 h-10 rounded-full ${getAvatarColor(partner?.name)} flex items-center justify-center text-white font-bold shadow-sm`}>
                                                {partner?.name?.charAt(0) || "?"}
                                            </div>
                                            <div>
                                                <h2 className="font-semibold text-slate-800 leading-tight text-sm">{partner?.name || "Student"}</h2>
                        ...
                        <div className="bg-white z-10 shrink-0 border-t border-slate-200">
                            <div className="p-3">
                                ...
                                <form onSubmit={handleSendMessage} className="flex items-end gap-2 max-w-4xl mx-auto">
                                    <button type="button" onClick={() => fileInputRef.current?.click()} className={`p-2.5 rounded-full transition-all duration-200 ${selectedImage ? 'bg-indigo-100 text-indigo-600' : 'text-slate-500 hover:bg-slate-200'}`}>
                                    ...
                                    <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col justify-end">
                                        ...
                                        {newMessage.length > 150 && (
                                            <div className="text-[10px] text-slate-400 font-medium px-4 pb-2 text-right">{newMessage.length}/200</div>
                                        )}
                                    </div>
                                    <button type="submit" disabled={!newMessage.trim() && !selectedImage} className="w-12 h-12 shrink-0 rounded-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 flex items-center justify-center text-white transition-colors shadow-sm">
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
                        ...
                        <p className="text-sm text-slate-500 mb-6">This will remove the conversation for you. Are you sure?</p>
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setShowDeleteConfirm(false)} disabled={isDeletingChat} className="px-4 py-2 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-100 transition-colors">
                                Cancel
                            </button>
                            <button onClick={handleDeleteConversation} disabled={isDeletingChat} className="px-4 py-2 rounded-xl text-sm font-medium bg-red-500 hover:bg-red-600 text-white transition-colors disabled:opacity-50">
