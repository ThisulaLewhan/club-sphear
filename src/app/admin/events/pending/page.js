    ...
    const handleAction = async (id, status) => {
        ...
        try {
            ...
            if (res.ok && json.success) {
                // Remove the handled event from the local list
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-black p-6 md:p-12">
            <div className="max-w-5xl mx-auto">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 pb-6 border-b border-zinc-200 dark:border-zinc-800 gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-zinc-900 dark:text-white">Admin Approval Queue</h1>
                        <p className="text-zinc-500 dark:text-zinc-400 mt-1">Review and approve pending club events</p>
                    </div>
                    <Link href="/" className="px-4 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-lg hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors font-medium">
                        Return to Calendar
                    </Link>
                </div>
